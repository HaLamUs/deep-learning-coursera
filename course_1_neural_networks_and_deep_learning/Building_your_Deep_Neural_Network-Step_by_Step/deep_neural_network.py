...
def linear_forward(A, W, b):
    ...
    Z = np.dot(W, A) + b
    # print(f"W { W.shape } A {A.shape} Z {Z.shape} b {b.shape}")
    ### END CODE HERE ###
...
def linear_backward(dZ, cache):
    ...
    """
    A_prev, W, b = cache # print(cache)
    m = A_prev.shape[1]

    ### START CODE HERE ### (≈ 3 lines of code)
    # print(f"dZ {dZ.shape} A_prev {A_prev.shape} W {W.shape}")
    dW = (1/m) * np.dot(dZ, A_prev.T)
    db = np.squeeze(np.sum(dZ, axis=1, keepdims=True)) / m#(1/m) * np.sum(dZ, axis=1, keepdims=True)
