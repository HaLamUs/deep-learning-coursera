...
def forward_propagation(X, parameters):
    ...
    Z2 = tf.add(tf.matmul(W2, Z1), b2)                    # Z2 = np.dot(W2, Z1) + b2
...
def model(X_train, Y_train, X_test, Y_test, learning_rate = 0.0001,
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(num_epochs):
            # mỗi lần chạy lại 1 epoch xét lại = 0 // += không bị tăng dần 
        ...
        correct_prediction = tf.equal(tf.argmax(Z3), tf.argmax(Y)) # argmax lấy giá trị cuối (trong mảng) để predict
